...
// 1.
const depositSum = accous.flatMap(acc => acc.movements).filter(mov => mov > 0).reduce((sum, mov) => sum + mov, 0);

console.log(depositSum);

// 2.
const numDeposit1000 = accous.flatMap(acc => acc.movements).reduce((count, cur) =>
  cur >= 1000 ? ++count : count, 0);

console.log(numDeposit1000);
// Prefixed ++ operator
let a = 10;
// console.log(a++); // 10 (present pre value)
// console.log(a); // 11
console.log(++a); // 11
console.log(a); // 11

// 3.
const { deposits, withdrawals } = accous.flatMap(acc => acc.movements).reduce((sums, cur) => {
