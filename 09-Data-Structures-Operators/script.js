...
const sliceStr = str => str.slice(0, 3).toUpperCase();

for (const flight of flights.split("+")) {
  const [type, form, to, time] = flight.split(";");
  const str = `${type.startsWith("_Delayed") ? "🔴" : ""}${type.replaceAll("_", " ")} form ${sliceStr(form)} to ${sliceStr(to)} (${time.replace(":", "h")})`.padStart(45);
  console.log(str);
}
*/